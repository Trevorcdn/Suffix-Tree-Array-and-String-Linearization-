...
Edge Edge::findEdge(int node, int asciiChar) {
 ...
 // that this can be used in the next iteration.
void suffixTree::migrateToClosestParent() {
   ...
   //     cout << "Nothing needs to be done for migrating" << endl;
        ...
        // adding above.
        if(e.startNode == -1) {
           // cout <<  rootNode << " <- rootNode " << startIndex << "<< Start Index " << Input[startIndex] << endl;
        }
 ...
 * iteration.
 */
// int counter = 0; //used to debug carryPhase
 void carryPhase(suffixTree &tree, int lastIndex, string T) {
 // void carryPhase(suffixTree &tree, int lastIndex){ 
 
    inputLength = T.length() - 1;
    nodeArray = (Node *)malloc(2*inputLength*(sizeof (Node)));
    Input = T;
   
    //inputLength = Input.length() - 1;
    //cout << "*********************In carryPhase()*************************" << endl;
    //cout << " Phase " << lastIndex << " Adding " << Input.substr(0, lastIndex + 1) << endl;
    // << "input length: "<< inputLength << "counter: "<< counter
    ...
    while (true) {
        ...
        if (tree.endReal() ) {
            //cout << "This is Input[lastIndex] in CarryPhase: " << Input[lastIndex]<<endl;
            //cout << "tree.rootNode: " << tree.rootNode <<endl;
        ...
        // We have not matching edge at this point, so we need to create a new
...
bool search(string pattern) {
    ...
    //cout<<"Seach: pattern= "<<pattern<<endl;
    int iter = 0;
    int i = -1;
    if (e.startNode != -1) {
        while(i < len) {
